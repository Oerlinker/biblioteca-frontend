                    ...
                    {isLoggedIn ? (
                        <>
                            <button onClick={toggleDropdown}
                                    className="hover:text-gray-200 font-semibold focus:outline-none flex items-center">
                                {user.nombre} <span className="ml-2 text-sm">&#9662;</span>
                            </button>
                            {isDropdownOpen && (
                                <div
                                    className="absolute right-0 mt-8 w-48 bg-white text-gray-800 rounded-md shadow-lg py-2 z-20"
                                    style={{top: '100%'}}>
                                    <Link to="/account" className="block px-4 py-2 hover:bg-gray-100">Gestionar
                                        Cuenta</Link>
                                    <button
                        ...
                        <nav className="flex flex-col p-4 space-y-4">
                            ...
                            {isLoggedIn && user.rol === 1 && !user.isSubscribed && (
                                <Link to="/subscription" onClick={toggleMenu}
                                      className="hover:text-gray-300">Suscripción</Link>
                            )}
                            {isLoggedIn ? (
                                <>
                                    <Link to="/account" onClick={toggleMenu} className="hover:text-gray-300">Gestionar
                                        Cuenta</Link>
                                    <button
                                        onClick={() => {
                                ...
                                <>
                                    <Link to="/login" onClick={toggleMenu} className="hover:underline">Iniciar
                                        Sesión</Link>
                                    <Link to="/register" onClick={toggleMenu} className="hover:underline">Crear
                                        Cuenta</Link>
                                </>
                            )}
                        </nav>
